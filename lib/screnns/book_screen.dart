...
import '../services/get_trips_api.dart';
  ...
  void bottomSheet(bool isFrom) {
    showModalBottomSheet(
      ...
      builder: (context) => BottomSheetPage(
        ...
        selectedCity: SelectedCity??fromCity,
      ),
    );
  }
              ...
              child: Padding(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    GestureDetector(
                      onTap: ()async {
                        if( fromCity!=S.of(context).Select_travel_city&&toCity!=S.of(context).Select_travel_arrive||toCode!=null&&fromCode!=null)
                        {
                          isErrorFrom=false;
                          isErrorTo=false;
                          setState(() {
                         ...
                         List result=await TripService().searchTrips(from: fromCode!, to: toCode!, date: "${selectedDate!.month}-${selectedDate!.day}-${selectedDate!.year}", chairs: numberOfSet.toString());
                          isLoading=false;
                          setState(() {

                          });
                         Navigator.push(context, MaterialPageRoute(builder: (context) =>result.isEmpty? ErrorSearch():TripsScreen(
                            fromCity: SelectedCity!,
                            toCity: SelectedToCity!,
                            fromCode: fromCode!,
                            toCode: toCode!,
                            result: result,
                            Chaires:numberOfSet.toString(),
                            SelectedDate:"${selectedDate!.month}-${selectedDate!.day}-${selectedDate!.year}",
                          ),));
